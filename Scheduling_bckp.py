...
class SchedulingManager:
    ...
    def CheckSlot(self,resource,job):
        ...
        for slot in resource.getEmptySlots():    
            ...
            self.getVisualManager().getSchedulingTab().getPSchScheRes().value+=" slot start Shift: "+str(curr_shift.getDay())+"-"+str(curr_shift.getNumber())+" r: "
    ...
    def MakeSchedule(self,b):
        ...
        while len(SchedulableJobs) >0:
            ...
            for j in SchedulableJobs:
                ...
                schreturn = self.CheckSlot(myresource,j)
                if schreturn == None: 
                    ...
                    if myresource.getMachineGroup() != None:
                        if myresource.getMachineGroup().getOperatingTeam() != None: 
                            fteuse = self.CalculateFTEUse(myresource,newslot[1])
                            ...
                            if fteuse > ftecapacity:
                                self.getVisualManager().getSchedulingTab().getPSchScheRes().value+=" FTE use XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX"+"\n"
                            self.getVisualManager().getSchedulingTab().getPSchScheRes().value+=" FTE use in start shift: "+ str(fteuse)+", capacity: "+str(ftecapacity)+"\n"
        ...
        self.getVisualManager().getSchedulingTab().getPSchSolProps().value = "Scheduled jobs: "+str(allscheduled)+"/"+str(nrjobs)+"\n"
