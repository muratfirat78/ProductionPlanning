...
############################################################################################
    def RunSimulation(self,OperationsMgr): 
        ...
        try: 
            ...
            self.locationDf = self.locationDf.reset_index().rename(columns={"index": "row_id"})
            self.locationDf = self.locationDf.sort_values(by=["PN", "Q", "Deadline", "row_id"], kind="stable")
            self.locationDf = self.locationDf.drop(columns=["row_id"]).reset_index(drop=True)
            self.getController().getVisualManager().updateSimProgress("Hoi2")
            self.processingDf = self.processingDf.reset_index().rename(columns={"index": "row_id"})
            self.processingDf = self.processingDf.sort_values(by=["PN", "Q", "Deadline", "row_id", "StartTime"], kind="stable")
            self.processingDf = self.processingDf.drop(columns=["row_id"]).reset_index(drop=True)
            self.getController().getVisualManager().updateSimProgress("Hoi3")
            self.locationDf.to_csv('LocationTest.csv', index=False)
            self.getController().getVisualManager().updateSimProgress("Hoi4")
            self.processingDf.to_csv('processingTest.csv', index=False)
